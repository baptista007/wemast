...
class Uploader {
    ...
    private function validate($file = null) {
        ...
        if ($file == null) {
            ...
            if (!file_exists($options['uploadDir']) && !is_dir($options['uploadDir']) && mkdir($options['uploadDir'], 0777, true)) {
            ...
            if ($file['type'][0] == "image" && @!is_array(getimagesize($file['tmp'])))
                $errors[] = $this->error_messages['accept_file_types'];
            if ($options['maxSize'] && $file['size'][0] > $options['maxSize'])
                $errors[] = $this->error_messages['max_file_size'];
    ...
    private function generateFileName($conf, $file, $skip_replace_check = false) {
        ...
        $random_string = substr(str_shuffle("_0123456789abcdefghijklmnopqrstuvwxyz"), 0, $length ? $length : 15);
        ...
        switch ($type) {
            ...
            default:
                ...
                if (strpos($string, "{{dirlistsequence}}") !== false) {
                    $string = str_replace("{{dirlistsequence}}", get_dir_file_count($this->options['uploadDir']), $string);
                }
                ...
                if (strpos($string, "{{date}}") !== false) {
                    $string = str_replace("{{date}}", date('d-D-M-Y_H-i-sa'), $string);
                }
                ...
                if (!empty($this->options['filenameprefix'])) {
                    $string = $this->options['filenameprefix'] . $string;
                }
        }
